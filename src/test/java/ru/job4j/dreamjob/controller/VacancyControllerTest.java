...
public class VacancyControllerTest {
    ...
    @Test
    void whenGetVacancyByIdThenOk() throws Exception {
        var vacancy = new Vacancy(1, "test1", "desc1", now(), true, 1, 2);
        var fileDto = new FileDto(testFile.getOriginalFilename(), testFile.getBytes());
        var expectedCities = List.of(new City(1, "Test"), new City(2, "Test2"));
        ...
        assertThat(actualVacancy).usingRecursiveAssertion().isEqualTo(vacancy);
        assertThat(cities).isEqualTo(expectedCities);

    }

    @Test
    void whenGetVacancyByIdThenError() throws Exception {
    ...
    @Test
    void whenPostUpdateThenOk() throws Exception {
        var vacancy = new Vacancy(1, "test1", "desc1", now(), true, 1, 2);
        var fileDto = new FileDto(testFile.getOriginalFilename(), testFile.getBytes());
        var fileDtoArgumentCaptor = ArgumentCaptor.forClass(FileDto.class);
        when(vacancyService.update(eq(vacancy), fileDtoArgumentCaptor.capture())).thenReturn(true);

        var model = new ConcurrentModel();
        var view = vacancyController.update(vacancy, testFile, model);
        var actualFile = fileDtoArgumentCaptor.getValue();

        assertThat(view).isEqualTo("redirect:/vacancies");
        assertThat(fileDto).usingRecursiveComparison().isEqualTo(actualFile);
    }

    @Test
    void whenPostUpdateThenError() throws Exception {
        var vacancy = new Vacancy(1, "test1", "desc1", now(), true, 1, 2);
        var fileDto = new FileDto(testFile.getOriginalFilename(), testFile.getBytes());
        var fileDtoArgumentCaptor = ArgumentCaptor.forClass(FileDto.class);
        when(vacancyService.update(eq(vacancy), fileDtoArgumentCaptor.capture())).thenReturn(false);
    ...
    @Test
    void whenPostUpdateThenThrowException() {
        var expectedException = new RuntimeException("Test message");
